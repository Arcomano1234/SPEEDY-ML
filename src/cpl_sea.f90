subroutine ini_sea(istart)
    ...
    use mod_io, only : write_netcdf_2d

    implicit none

    integer, intent(in) :: istart
    real, allocatable :: diff(:), temp(:), temp2d(:,:)
    real :: bias
    ...
    !sstcl_ob = sstcl_ob + 2.0 !THIS NEEDS TO BE CHANGED TROY SST TODO NOTE
    sst_om(:)  = sstcl_ob(:)      ! SST 
    ...
    if(internal_state_vector%hybrid_slab) then
