subroutine invars 
    ...
    use mod_tsteps, only: iyear0, imont0, istart, era_start
    ...
    if (istart.eq.0) then
        ...
        ! Print diagnostics from initial conditions
        call diagns (2,0)
    else if ((istart .eq. 2).and.(era_start == 0)) then
        ! 4. Start from grid initial condition

        call iogrid(27)
        call diagns(1,0)
    else if ((istart .eq. 2).and.(era_start == 1)) then
        ! 5. Start from grid era_5 re_analysis

        call iogrid(28)
        call diagns(1,0)
    else if ((istart .eq. 2).and.(era_start == 2)) then
        ! 6. Regrid era data

        call iogrid(29)
        call diagns(1,0)
    else if((istart .eq. 2).and.(era_start == 3)) then
        !7. hybrid machine learning!!!
